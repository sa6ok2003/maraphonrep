...
async def admin_ka(message: types.Message):
    ...
    if id in ADMIN_ID:
        ...
        bat_b = types.InlineKeyboardButton(text='Рассылка, кто прошел марафон', callback_data='write_message')
        bat_с = types.InlineKeyboardButton(text='Рассылка, кто не прошел марафон', callback_data='write_message0')
...
async def rassilka (call:types.callback_query,state: FSMContext):
    ...
    murkap.add(bat0)
    await bot.send_message(call.message.chat.id,'Перешли мне уже готовый пост и я разошлю тем кто прошел марафон',reply_markup=murkap)
    await reg.name.set()


@dp.message_handler(state=reg.name,content_types=['text','photo','video','video_note','voice'])
...
async def rassilka0 (call:types.callback_query,state: FSMContext):
    ...
    bat0 = types.InlineKeyboardButton(text='ОТМЕНА',callback_data='otemena')
    murkap.add(bat0)
    await bot.send_message(call.message.chat.id,'Перешли мне уже готовый пост и я разошлю его всем юзерам кто не прошел марафон',reply_markup=murkap)
    await reg0.name0.set()


@dp.message_handler(state=reg0.name0,content_types=['text','photo','video','video_note','voice'])
